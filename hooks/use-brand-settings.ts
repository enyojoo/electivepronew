...
export function useBrandSettings(): BrandSettings {
  ...
  const [confirmedNoCustom, setConfirmedNoCustom] = useState(false)
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        ...
        setConfirmedNoCustom(confirmed === "true" && hasCustomFlag === "false")
      } catch {
  ...
  const getValue = (
    ...
    if (!hasConfirmedData && typeof document !== "undefined") {
      ...
      // Return empty string during loading - never show defaults until confirmed
      return ""
    }

    // If we've confirmed no custom branding exists, use default for both languages
    if (hasConfirmedData && confirmedNoCustom && !hasCustomBranding) {
      return defaultValue
    }

    // Determine which value to use based on language
    // Use Russian value when language is Russian, English value when English
    // If language-specific value is not set, fallback to the other language's value
    const customValue = language === "ru" 
      ? (customValueRu || customValueEn)
      : (customValueEn || customValueRu)
    ...
    // If we haven't confirmed yet, return empty string
    if (!hasConfirmedData) {
      return ""
    }

    // Fallback to default only if confirmed no custom branding
    return confirmedNoCustom ? defaultValue : ""
  }
  ...
  const getLogoUrl = (): string => {
    ...
    // If no custom branding exists and we've confirmed, use default for both languages
    if (hasConfirmedData && confirmedNoCustom && !hasCustomBranding) {
    ...
    // Fallback to default only if confirmed no custom branding
    return confirmedNoCustom ? DEFAULT_LOGO_URL : ""
  }
